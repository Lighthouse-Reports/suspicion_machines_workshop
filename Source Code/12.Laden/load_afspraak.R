...
read_feather(paste0(bronfolder, tabellen$file_def[tabellen$tabel == tabel])) %>%
  ...
  select(-c(X_WEIFRA_AFSPRAAK_ID, MTG_BRON, MTG_DATUM, MTG_LOB_ID, MTG_GELDIG_VAF, MTG_GELDIG_TEM)) %>%
  # Overige omzettingen
  set_names(tolower(names(.))) %>%
  rename(jn_nr = afspraak_nr,
         persoon_id = bsn) %>% 
  mutate(datum_afgehandeld = as_date(datum_afgehandeld)) %>%
