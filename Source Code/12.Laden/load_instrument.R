...
read_feather(paste0(bronfolder, tabellen$file_def[tabellen$tabel == tabel])) %>% 
  ...
  select(-c(X_WEIFRA_INSTRUMENT_ID, MTG_BRON, MTG_DATUM, MTG_LOB_ID, MTG_GELDIG_VAF, MTG_GELDIG_TEM)) %>%
  # Overige omzettingen
  set_names(tolower(names(.))) %>%
  rename(jn_nr = volgnr,
         persoon_id = bsn) %>%
  mutate(datum_invoer = as_date(datum_invoer),
         dtaanvang = as_date(datum_aanmelding),
