...
read_feather(paste0(bronfolder, tabellen$file_def[tabellen$tabel == tabel])) %>%
  ...
  select(-c(X_WEIFRA_TYPERING_ID, MTG_BRON, MTG_DATUM, MTG_LOB_ID, MTG_GELDIG_VAF, MTG_GELDIG_TEM)) %>%
  # Overige omzettingen
  set_names(tolower(names(.))) %>%
  rename(jn_nr = klant_typering_nr,
         persoon_id = bsn) %>%
  mutate(dtaanvang = as_date(datum_ingang),
