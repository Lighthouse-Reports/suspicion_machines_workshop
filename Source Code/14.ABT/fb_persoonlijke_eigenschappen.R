...
varlist_behouden <- c("spreektaal", "ind_regulier_arbeidsritme", 
...
varlist_behouden_nieuw <- c("vulling", "dagen_sinds_opvoer", "dagen_sinds_taaleis")

abt_pers_eig <- df_pers_eig %>%
  mutate(vulling = as.integer(rowSums(!is.na(select_(., .dots = c("-target_per"))))),
  ...
  select_(.dots = c("id", "jn_datetime", varslist_isna, varslist_isfactor, varlist_behouden, varlist_behouden_nieuw)) %>%
  group_by(id) %>%
  arrange(desc(jn_datetime), desc(vulling)) %>% 
  slice(1) %>%
  ungroup() %>%
  select(-jn_datetime)
